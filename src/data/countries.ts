...
export function findCountry(code:string): TCountry | undefined {
  return countries.find(item => item.code === code);
}

export function africanCountries(): TCountry[] {
  return countries.filter(item => african.includes(item.name.toLowerCase()));
}

const african: string[] = [
